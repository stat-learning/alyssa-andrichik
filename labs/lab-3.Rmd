...
```{r}
goodershit <- d %>%
  select(-c(state, county, community, communityname, population, LemasSwornFT, LemasSwFTPerPop, LemasSwFTFieldOps, LemasSwFTFieldPerPop, LemasTotalReq, LemasTotReqPerPop, PolicReqPerOffic, PolicPerPop, RacialMatchCommPol, PctPolicWhite, PctPolicBlack, PctPolicHisp, PctPolicAsian, PctPolicMinor, OfficAssgnDrugUnits, NumKindsDrugsSeiz, PolicAveOTWorked, PolicCars, PolicOperBudg, LemasPctPolicOnPatr, LemasGangUnitDeploy, PolicBudgPerPop)) %>%
  drop_na()
view(goodershit)
...
geom_point(alpha = 0.2) +
  labs(x = "percentage of kids born to never married", y = "total number of violent crimes per 100K popuation")
```
...
y = ViolentCrimesPerPop)) +
geom_point(alpha = 0.2) +
  labs(x = "percentage of population that is caucasian", y = "total number of violent crimes per 100K popuation")
```
...
y = ViolentCrimesPerPop)) +
geom_point(alpha = 0.2) +
  labs(x = "percentage of households with public assistance income in 1989", y = "total number of violent crimes per 100K popuation")
```
...
R Functions
1. Fitting the model
Please write a function that takes the training data as input and then outputs an lm object. It should be of the form,
...
x <- sample(c(TRUE, FALSE), 800, replace = TRUE)
  train <-d[!x,]
  test <-d[x,]
  m1<-lm(ViolentCrimesPerPop ~ PctIlleg + racePctWhite + PctPersOwnOccup, data = train)
}
summary(m1)
```
...
MSE
    ...
    group_K_MSE(model = m1, data = test)
```

When you turn in this lab, you should be running group_A_MSE on the training data to get training MSE. I’ll be 
changing out the data set when I run this function to find your groups’ test MSE.